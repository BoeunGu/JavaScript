...
// const newFriends = friends.slice(friends.findIndex(f=> (f.city.indexOf('seoul'))>-1)) // findIndex는 판별함수의 조건이 맞아야 그에 해당하는 인덱스 값을 리턴해줌
...
// const modifiedSentence = lyricsArray.map(word=> {
//     if(word.includes('shit')) {word = `💩`}
...
// const novelArray = novel.split("");

// const changedOne = novelArray
//   .map((a) => a.toLowerCase())
//   .filter((a, index, array) => {
//     if (!specialCharactersArray.includes(a) && a !== " " && a !== "\n")
//       return a;
//   })
//   .sort();

// changedOne.forEach((key) => {
//   if (!classified[key]) classified[key] = 0;
//   classified[key]++;
// });

// for (let prop in classified) {
//   alphabetFrequency.push({ alpha: prop, cnt: classified[prop] });
// }
...
function  highlightKeyword(e){
  const keyword = e.target.value //사용자 검색어 입력값
  ...
  if(keyword !== ''){
    ...
    displaylyrics(modifiedlyrics)  // 하이라이트 적용된 가사 보여주기
  }
  else{
    //화면에 그냥 가사 디스플레이
